...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        not_terminal = ptu.from_numpy(1 - terminal_n)
        cntr = loss = 0


        for _ in range(self.num_target_updates * self.num_grad_steps_per_target_update):

            if not cntr:
                targets = reward_n + (self.gamma * self.forward(next_ob_no) * not_terminal)
                targets = targets.detach()

            predictions = self.forward(ob_no).squeeze()
            curr_loss = self.loss(predictions, targets)
            loss += curr_loss
            ...
            cntr = (cntr + 1) % self.num_grad_steps_per_target_update

        return loss / (self.num_target_updates * self.num_grad_steps_per_target_update)
